simulacao = function(bv1=1,bv2=1,bv3=1,
                     av1=1,av2=1,av3=1,
                     alfa=1,i=500,seed=1,
                     plt=TRUE, plty="l", transp=1, lwd=1,
                     ymin=0, ymax=1){
  ...
  while(t<=i){
      ...
      pb1 = exp(-alfa*av1)/(exp(-alfa*av1)+exp(-alfa*av2))
      pb2 = 1-pb1
      ##Probabilidade das azuis
      pa1 = exp(-alfa*bv1)/(exp(-alfa*bv1)+exp(-alfa*bv2))
      pa2 = 1-pa1
      ...
      ##--------------
      ##Probabilidade das brancas
      pb2 = exp(-alfa*av2)/(exp(-alfa*av2)+exp(-alfa*av3))
      pb3 = 1-pb2
      ##Probabilidade das azuis
      pa2 = exp(-alfa*bv2)/(exp(-alfa*bv2)+exp(-alfa*bv3))
      pa3 = 1-pa2    
      ##gerando um valor aleatório entre 0 e 1 para as bolas brancas
      valor = runif(1,0,1)
      ##Verificando para qual vértice as bolas brancas irao
      ...
      ##--------------
      ##Probabilidade das brancas
      pb1 = exp(-alfa*av1)/(exp(-alfa*av1)+exp(-alfa*av3))
      pb3 = 1-pb1
      ##Probabilidade das azuis
      pa1 = exp(-alfa*bv1)/(exp(-alfa*bv1)+exp(-alfa*bv3))
      pa3 = 1-pa1
      ##gerando um valor aleatório entre 0 e 1 para as bolas brancas
      valor = runif(1,0,1)
      ##Verificando para qual vértice as bolas brancas irao
    ...
    if (plt !=0) {
        ...
        if (plty != "t") { 
            par(mfrow = c(1,2))
            plot(0,1/3, xlim=c(1,i), ylim=c(ymin,ymax), type = "l", xlab="n",
                 ylab="X(n)")
        ...
        # grafico dos caminhos no simplex
        if (plty != "l") {
